...
// This function is for creating tables in the databasee
...
int db_insert_message(sqlite3 *db, const char *username, const char *message)
{
    ...
    sqlite3_bind_text(stmt, 1, username, -1, SQLITE_TRANSIENT); // Binds the username to the first '?' in the SQL statement. SQLITE_TRANSIENT makes a copy of the string.
    sqlite3_bind_text(stmt, 2, message, -1, SQLITE_TRANSIENT);   // Binds the message to the second '?' in the SQL statement. SQLITE_TRANSIENT makes a copy of the string.
